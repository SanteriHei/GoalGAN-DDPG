...
def _clean_up(avg_coverages: npt.NDArray, rewards: npt.NDArray, gan: LSGAN, agent: DDPGAgent) -> None:
    ...
    np.save("checkpoints/avg_rewards.npy")
...
def _update_or_eval_policy(
    ...
    timestep_count: int, eval_mode: bool = False, global_step: Optional[int] = None,
    ...
    if eval_mode:
        return env.achieved_goals/(policy_iter_count*episode_count)
...
def _create_goals(
    ...
    goal_counts: Dict[str, int]
        Defines the amount of goals coming from each source. 
        Values "gan" and "random" should provided.
    
    Returns
    -------
    Tuple[torch.Tensor, torch.Tensor]
        Tuple containing the generated set of goals and the goals generated by gan. Located on same device as the GAN.
    '''
    gan_goal_count = goal_counts.get("gan", None)
    assert gan_goal_count is not None, "No GAN goal count provided"

    random_goal_count = goal_counts.get("random", None)
    assert random_goal_count is not None, "No count was specified for random goals"
...
def train(
    ...
    for i in range(iter_count):
        ...
        if (i + 1)%10 == 0:
            utils.display_agent_and_goals(
                ...
                filepath=f"images/goals_iter_{i}.png", pos_label="Agent position",
                 title=f"Iteration {i}", goal_label="goals"
            )
            _logger.info(f"Displayed agent and goals")

        goals, gan_goals = _create_goals(gan, old_goals, env.goal_size, {"gan": gan_goal_count, "random": random_goal_count})

        env.goals = goals.detach().cpu().numpy()

        #Update the agent.
        _logger.info("Starting updating the policy")
    ...
    _clean_up(avg_coverages,rewards, gan, agent, iter_count)
